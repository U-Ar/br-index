...
void help()
{
	cout << "bri-locate2: locate all occurrences of the input patterns with exactly two mismatches." << endl << endl;

	cout << "Usage: bri-locate2 [options] <index> <patterns>" << endl;
...
void locate_all(ifstream& in, string patterns)
{
    ...
    for (ulint i = 0; i < n; ++i)
    {
        ...
        vector<ulint> occs = idx.locate2(p);

        if (c) // check occurrences
        {
            cout << "number of occs : " << occs.size() << endl;
            for (auto o : occs)
            {
                int mismatches = 0;
                ...
                if (mismatches != 2) 
                {
                    ...
                    cout << "       wrong    pattern: " << text.substr(o-1,p.size()+1) << endl;
                }
            }
        }
    }
